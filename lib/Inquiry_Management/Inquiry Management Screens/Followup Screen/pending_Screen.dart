...
class _PendingScreenState extends State<PendingScreen> with SingleTickerProviderStateMixin {
  ...
  String? currentStage;
  String currentFollowUpDay = 'pending'; // Default tab
  int currentStatus = 1; // 1 for Live, 5 for Due
  int currentStatuscnr = 1;

  String selectedMainFilter = "Live";
  int selectedButtonIndex = 1; // 1 for List View, 2 for Stages View
  ...
  Future<void> loadAllInquiriesFollowup() async {
    ...
    setState(() {
      ...
      // Preserve the current selectedList if it’s not "All", otherwise default to "All"
      if (selectedList == "All" || selectedList.isEmpty) {
        selectedList = "All";
        selectedValue = inquiryProvider.fullStageCountFollowup["Total_Sum"] ?? "0";
      } else {
        // Reapply the current selectedList and update selectedValue accordingly
        selectedValue = inquiryProvider.fullStageCountFollowup[selectedList] ?? "0";
      }

      selectedIndex = 0; // Reset selectedIndex if needed
                ...
                Padding(
                  ...
                  child: PopupMenuButton<int>(
                    ...
                    child: Container(
                      ...
                      child: Icon(
                        selectedButtonIndex == 1 ? Icons.list : Icons.grid_view,
                        color: Colors.black,
                        size: 20,
                      ),
                    ),
              ...
              child: Builder(
                builder: (context) {
                  if (inquiryProvider.isLoadingFollowup && inquiryProvider.inquiriesFollowup.isEmpty) {
                    ...
                    if (selectedButtonIndex == 1) { // List View
                      return ListView.builder(
                        physics: AlwaysScrollableScrollPhysics(),
                    ...
                    } else { // Stages View (selectedButtonIndex == 2)
